 ...
 *
 * Разбор сохраняется в localStorage рядом с ответами — но только его открытая
 * часть: блоки 4–10 сервер придерживает до оплаты. Если запрос не удался,
...
export function AnalyzingStep({
  ...
  useEffect(() => {
    ...
    void (async () => {
      const { analysis, reportId, sealed } = await fetchAnalysis(state);
      if (cancelled) return;

      // Сохраняем и когда разбора нет: запечатанная закрытая часть считается и
      // без нейросети, а без неё страница отчёта не сможет её открыть.
      if (analysis || sealed) saveAnalysis(state, { analysis, reportId, sealed });
