...
/** «1 match-gasoline (Спичка и бензин)» — и номер, и смысл, и подпись. */
function catalogue(
  ids: readonly string[],
  names: Record<string, { name: string }>,
): string {
  return ids
    .map((id, index) => `${index + 1} ${id} (${names[id].name})`)
    .join("\n");
}
...
{"a":N,"p":N,"r":N,"c":N,"m":{${METRIC_KEYS.map((key) => `"${key}":N`).join(",")}},"b":{${BATTLE_KEYS.map((key) => `"${key}":N`).join(",")}},"ab":"nobody|she|he|both","f":N,"s":"...","an":"..."}
...
Оба текста на ${LANGUAGE_NAME[locale]} языке, живым тоном, без кавычек, эмодзи и переносов строк.
