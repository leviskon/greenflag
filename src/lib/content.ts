...
export const HERO_FACTS = ["minutes", "insights", "shared"] as const;

/** Пример флагометра в карточке первой секции. */
...
/**
 * Якорь блока с кнопкой в первой секции.
 *
 * Живёт здесь, а не в компоненте: id нужен и серверному Hero, и клиентской
 * липкой кнопке, а тянуть константу из клиентского модуля в серверный нельзя.
 */
export const HERO_CTA_ID = "hero-cta";

/** Ссылки меню: только те разделы, что остались на странице. */
